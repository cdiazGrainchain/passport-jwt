...
PassportConfigurator.prototype.setupModels = function(options) {
  ...
  //this.userModel = loopback.getModelByType(this.app.models.Member);
   let m ="Member"
  this.userModel = loopback.getModel(m);
  ...
  switch (authType) {
    ...
    case 'saml-gc':
      ...
      strategy = new AuthStrategy(_.defaults({
         usernameField: options.usernameField || 'username',
         passwordField: options.passwordField || 'password',
         modelField: options.modelField || 'model',
         tokenField: options.tokenField || 'token',
         passReqToCallback: true,
      }, options),
        function(req, profile, done, username, password, model, token) {
          if(model=='None'){
              console.log(1);
              //var ptoken = profile.toJSON();
              console.log(profile);
              if(profile.username=='CAS'){
                  console.log(2);
                  self.userIdentityModel.login(name, authScheme, profile, {}, options, loginCallback(req, done));
              }else{
                  return done();
              }
          }
          ...
          self.userModel = loopback.findModel(model);
          
          if(typeof self.userModel === "undefined"){
              return done();
          }
          self.userModel.findOne(query, function(err, user) {
              if (!user)
