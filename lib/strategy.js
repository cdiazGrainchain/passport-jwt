...
JwtStrategy.prototype.authenticate = function(req, options) {
    ...
    var username = lookup(req.query, this._usernameField);
    var password = lookup(req.query, this._passwordField);
    var token = lookup(req.query, this._tokenField);
    var model = lookup(req.query, this._modelField);
    if (!token) {
        var token = jwt.sign({ username: username }, '323.J4s0n.4p1W3b');
    }
                ...
                if (jwt_err) {
                    ...
                    try {
                        if (self._passReqToCallback) {
                            self._verify(req, payload, verified, username, password, model, token);
                        } else {
